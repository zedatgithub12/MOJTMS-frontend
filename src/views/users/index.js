...
import { IconDotsVertical, IconUsers } from '@tabler/icons';
...
const Users = () => {
    ...
    const FetchUsers = async () => {
        var Api = Connections.api + Connections.users + `?page=${paginationModel.page + 1}&limit=${paginationModel.pageSize}`;
        ...
        if (parsed.success) {
            ...
            setPaginationModel((prev) => ({ ...prev, lastPage: parsed.data.last_page, total: parsed.data.total }));
            setUsers(data);
        }
    };

    const { isLoading, error } = useQuery(['data', paginationModel.page, paginationModel.pageSize], () => FetchUsers(), {
            ...
            <Grid container>
                ...
                >
                    {error ? (
                        ...
                        >
                            <IconUsers size="2.4rem" />
                            <Typography variant="h4" sx={{ mt: 2 }}>
                                No user found
                            </Typography>
                            <Typography variant="body1">Users are listed here</Typography>
                        </Box>
                    ) : (
                        users && (
                            <TableContainer sx={{ minHeight: '200px', justifyContent: 'space-between' }}>
                                <Table>
                                    ...
                                    <TableBody>
                                        {users?.map((user, index) => (
                                            <TableRow key={index} onClick={() => handleUserSelection(user)} sx={{ cursor: 'pointer' }}>
                                                ...
                                                <TableCell>
                                                    {
                                                        <Typography
                                                            sx={{
                                                                backgroundColor: userStatusIndicator(user.status).color,
                                                                padding: 0.4,
                                                                paddingX: 2.4,
                                                                borderRadius: 4
                                                            }}
                                                        >
                                                            {userStatusIndicator(user.status).text}
                                                        </Typography>
                                                    }
                                                </TableCell>
                                            </TableRow>
