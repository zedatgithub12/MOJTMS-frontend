...
const ViewSurvey = () => {
    ...
    const fetchSurvey = async () => {
        var Api = Connections.api + Connections.surveys + '/' + state.id;
    ...
    const handleSurveyStatus = (newStatus) => {
        setPublishing('processing');
        const Api = Connections.api + Connections.surveyStatus + state.id;
        const token = sessionStorage.getItem('token');
        const headers = {
            Authorization: 'Bearer' + token
        };
    ...
    const handleCreatingQuestion = (values) => {
        ...
        const Api = Connections.api + Connections.surveyquestions;
        const token = sessionStorage.getItem('token');
        const headers = {
            Authorization: 'Bearer' + token
        };
        ...
        data.append('survey_id', state.id);
        ...
        fetch(Api, { method: 'POST', headers: headers, body: data })
            ...
            .then((response) => {
                if (response.success) {
                    ...
                    values.question_type === 'fill' ? fetchSurvey() : setDisplay('option');
                } else {
                    setCreatingQuestion(false);
                    handlePrompts(response.message, 'error');
                }
    ...
    const handleOptionSubmission = (choices) => {
        ...
        const Api = Connections.api + Connections.surveyoptions;
        const token = sessionStorage.getItem('token');
        const headers = {
            Authorization: 'Bearer' + token,
            'Content-Type': 'application/json'
    ...
    //the following function handles deleting question
    const DeleteQuestion = (que_Id) => {
        setDeleting(true);

        var Api = Connections.api + Connections.surveyquestions + '/' + que_Id;
        const token = sessionStorage.getItem('token');
        var headers = {
            Authorization: `Bearer` + token,
            accept: 'application/json',
                ...
                {questions.map((question, index) => (
                    <Box key={question.id} sx={{ marginTop: 6, paddingX: 1 }}>
                        ...
                        <Box>
                            {question.question_type === 'multiple-choice' && question.surveyoptions ? (
                            ...
                            ) : question.question_type === 'fill' && question.surveyoptions ? (
                                <Box
                                    sx={{
                                        display: 'flex',
                                        flexDirection: 'row',
                                ...
                                >
                                    <Typography variant="subtitle2">Fill in blank space question</Typography>
                                </Box>
                            ) : (
