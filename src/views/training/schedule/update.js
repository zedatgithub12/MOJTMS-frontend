...
const UpdateSchedule = () => {
    ...
    const handleSubmitting = (values) => {
        ...
        const Api = Connections.api + Connections.schedules + '/' + state.id;
        const token = sessionStorage.getItem('token');
        const headers = {
            Authorization: 'Bearer' + token
        };
    ...
    const formik = useFormik({
        initialValues: {
            scheduled_title: state ? state.scheduled_title : '',
            scheduled_description: state ? state.schedule_description : '',
            startdate: state ? state.start_datetime : '',
            enddate: state ? state.end_datetime : ''
        },
