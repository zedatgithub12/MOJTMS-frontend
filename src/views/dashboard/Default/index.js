...
// material-ui
import { Typography, Grid, Skeleton, TableRow, TableCell, Table, TableHead, useTheme, TableBody, TextField } from '@mui/material';
...
import TrainingSummaryTable from './components/TrainingReport';
...
const Dashboard = () => {
    ...
    const handleGettingTrainingReport = async () => {
        setLoadingReport(true);
        var Api = Connections.api + Connections.training_report + `?start_date=${startDate}&end_date=${endDate}`;
        const token = sessionStorage.getItem('token');
        var headers = {
            Authorization: `Bearer` + token,
            accept: 'application/json',
                                ...
                                <Grid item xs={12}>
                                    <Grid container sx={{ display: 'flex', alignItems: 'center', p: 1 }}>
                                        <Grid item xs={12} sm={6}>
                                            <Typography variant="h4">Training Report</Typography>
                                        </Grid>
                                        <Grid
                                            item
                                            xs={12}
                                    ...
                                    <TrainingSummaryTable isLoading={loadingReport} data={report} />
                                </Grid>
                            </Grid>

                            <Grid container mt={3}>
                                <Grid item xs={12} sx={{ border: 0.6, borderColor: '#ddd', borderRadius: 4, py: 2, px: 1 }}>
                                    <Typography variant="subtitle1" marginLeft={2}>
                                        {t('Monthly, male and female trainees')}
                                    </Typography>
